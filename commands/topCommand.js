...
(function() {
    ...
    $.bind('command', function(event) {
        ...
        /**
         * @commandpath top5 - Get the top 5 viewers in the channel based on points - Viewer
         */
        if (command.equalsIgnoreCase('top5')) {
            if (!$.bot.isModuleEnabled('./systems/pointSystem.js')) {
                $.say($.whisperPrefix(sender) + $.lang.get('top5.points-disabled'));
                return;
            }
            ...
            for (i in temp) {
                top5.push(c + '. ' + $.resolveRank(temp[i].username) + ' ' + temp[i].value);
                c++;
            }
        ...
        /**
         * @commandpath top5time - Get the top 5 viewers in the channel based on time - Viewer
         */
        if (command.equalsIgnoreCase('top5time')) {
            ...
            $.say($.lang.get('top5.default', 'Zeit', top5.join(', ')));
        }
    });
